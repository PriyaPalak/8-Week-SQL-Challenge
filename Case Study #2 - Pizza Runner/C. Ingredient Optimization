# :pizza: Case Study #2 - Pizza Runner
  ...
  /* In this section, I first got my hands dirty with solving each query in bits and pieces. 
  Then, I realised there are certain results(tables) which are repeatedly used in many queries. So, I created them as temporary tables. 
  While solving the queries, I also realised that data type of certain TEXT columns needs to be converted to Character type (Nvarchar)
  so that the various String Functions(like STRING_SPLIT(), STRING_AGG(), and CONCAT_WS()) can be performed on them. */
  ...
  -- Data Type Transformations to be able to perform certain operations
   ...
   -- 2. Creating a temporary table #pizza_toppingss and changing the data type of 'topping_name' from text to NVARCHAR(100) to be able to perform STRING_AGG() on it.
  ...
  -- input_string(1st argument for this function) should be of the type NVARCHAR, VARCHAR, NCHAR or CHAR.
   ...
   SELECT	
   r.pizza_id,TRIM(t.value) AS topping_id,pt.topping_name  --TRIM() removes any leading or trailing spaces and makes sure the substrings are aligned properly.
  ...
  FROM 
   ...
   SELECT * FROM #pizzas_with_topppings;

   -- 2. Adding a column record_id to the table #customer_orders to select each pizza in an order individually
  ...
  SELECT pt.topping_name, COUNT(r.record_id) AS number_of_addition
  ...
  SELECT pt.topping_name, COUNT(r.record_id) AS number_of_removals
  ...
  GROUP BY pt.topping_name;
   ...
   -- CONCAT_WS() function concatenates two or more strings into one string with a separator. CONCAT_WS() means concatenate with separator.
   -- Again, the input_string should be NVARCHAR, VARCHAR etc.


	WITH exclusions_cte AS
	(
		SELECT r.record_id, 'Exclude ' + STRING_AGG(pt.topping_name, ', ') AS optimisation
		FROM #records_with_exclusions r
		JOIN #pizza_toppings pt
		ON r.exclusions = pt.topping_id
	...
	(
		SELECT r.record_id, 'Extra ' + STRING_AGG(pt.topping_name, ', ') AS optimisation
		FROM #records_with_extras r
		JOIN #pizza_toppings pt
		ON r.extras = pt.topping_id
		GROUP BY r.record_id
 ...
 (
	SELECT c.record_id, p.pizza_name,
	CASE WHEN pt.topping_id
	IN (SELECT extras FROM #records_with_extras r WHERE c.record_id = r.record_id) --Here, c.record_id = r.record_id to make sure we are giving 2x to extras for the specific records and not all the records
	...
	WHERE pt.topping_id NOT IN (SELECT exclusions FROM #records_with_exclusions r WHERE c.record_id = r.record_id) --Here, WHERE c.record_id = r.record_id is like there are some records which have exclusions and for those records , the topping_id should not be in the record's excluded list of toppings.

	)

	
	SELECT record_id, CONCAT_WS(' :', pizza_name, STRING_AGG(toppings, ', ')) AS ingredients_list
	...
	WITH count_cte AS
 (
	SELECT record_id,topping_name AS ingredient,
	...
	SELECT ingredient, SUM(times_used) AS total_count
	FROM count_cte
	GROUP BY ingredient
 ...
 1. Various new STRING Manipulation Functions like STRING_SPLIT(), STRING_AGG() , CONCAT with + and CONCAT_WS()
 2. The tables provided were not in the format that you can directly fetch the results from. I had to create many new tables, by combining the existing tables, in the desired formats to get the desired results.
 3. It was not easy to picture the required CTEs in mind as the designs were quite complex involving many tables and newly created temporary tables. 
